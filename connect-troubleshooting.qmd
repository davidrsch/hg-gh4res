...
You may be able to find Git after the fact with these commands in the shell (@sec-shell):
...
If you cannot deal with the two root causes identified above, then it is possible that a more powerful Git client (@sec-git-client) will be able to cope with these situations. But I make no promises. You should also try Git operations from the command line.
