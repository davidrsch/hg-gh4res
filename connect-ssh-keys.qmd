# Set up keys for SSH {#sec-ssh-keys}
...
Here we describe the credential setup for the SSH protocol. If you're not sure whether to use HTTPS or SSH, please read [HTTPS versus SSH](#https-vs-ssh). From now on, we assume you've made an intentional choice to set up SSH keys.
...
-   If you are new to programming and the shell, you'll probably find HTTPS easier at first (chapter @sec-https-pat). You can always switch to SSH later. You can use one method from computer A and the other from computer B.
...
## Troubleshooting {#sec-ssh-troubleshooting}
