...
Go to the shell (@sec-shell). Enter `which git` to request the path to your Git executable:
...
This also leaves you with a Git client, though not a very good one. So check out Git clients we recommend (@sec-git-client)).
...
After you install Chocolatey, in a shell (@sec-shell)), do:
