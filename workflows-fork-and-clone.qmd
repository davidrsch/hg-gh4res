...
Vocabulary: `OWNER/REPO` refers to what we call the **source** repo, owned by `OWNER`, who is not you. `YOU/REPO` refers to your fork, i.e. your remote copy of the source repo, on GitHub. This is the same vocabulary used elsewhere, such as the chapter on [common remote configurations](sec-common-remote-setups).
...
-   The [cloning instructions in Existing project, GitHub first](sec-git-clone-usethis-rstudio) cover usethis and RStudio.
-   The [cloning instructions in Connect to GitHub](sec-git-clone-command-line) show how to do this with command line Git.

Make a conscious decision about the local destination directory and HTTPS vs SSH URL.

### Finish the fork and clone setup {#sec-fork-and-clone-finish}

If you stop at this point, you have what I regard as an incomplete setup, described elsewhere as ["fork (salvageable)"](sec-fork_upstream_is_not_origin_parent).
...
These steps make it easier for you to stay current with developments in the source repo. We talk more below about why you should never commit to the default branch, e.g. `main`, when you're working in a fork (see @sec-fork-dont-touch-main).

### Configure the `upstream` remote {#sec-fork-configure-upstream}
...
### Set upstream tracking branch for the default branch {#sec-fork-set-upstream-tracking-main}
...
-   Configures the source repo as [the `upstream` remote](sec-upstream-changes).
...
If you used `usethis::create_from_github()` or did fork and clone via [Existing project, GitHub first](sec-existing-github-first), you are probably in an RStudio Project for this new repo.
...
## Don't mess with `main` {#sec-fork-dont-touch-main}

Here is some parting advice for how to work in a fork and clone and situation.

If you make any commits in your local repository, I **strongly recommend** that you work in [a new branch](sec-git-branches), not `main` (or whatever the default branch is called).

I **strongly recommend** that you do not make commits to `main` of a repo you have forked. If you commit to `main` in a repo you don't own, it creates a divergence between that branch's history in the source repo and in your repo. Nothing but pain will come from this. (If you've already done this, we discuss how to fix the situation in [Um, what if I did touch `main`?](sec-touched-main).)

When you treat `main` as read-only, it makes life much easier when you want to [pull upstream work](sec-upstream-changes) into your copy. The `OWNER` of `REPO` will also be happier to receive your pull request from a non-`main` branch.
