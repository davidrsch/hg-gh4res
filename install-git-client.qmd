# Install a Git client {#sec-git-client}
...
"Git" is really just a collection of individual commands you execute in the shell (Appendix @sec-shell). This interface is not appealing for everyone. Some may prefer to do Git operations via a client with a graphical interface.

Git and your Git client are not the same thing, just like R and RStudio are not the same thing. A Git client and an [integrated development environment](https://en.wikipedia.org/wiki/Integrated_development_environment), such as RStudio, are not necessary to use Git or R, respectively. But they make the experience more pleasant because they reduce the amount of "command line bullshittery"[^install-git-client-1] and provide a richer visual representation of the current state.

[^install-git-client-1]: This evocative phrase originally appeared in a blog post by Philip Guo, which has subsequently been removed from the internet.
