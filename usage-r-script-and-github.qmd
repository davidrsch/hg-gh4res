# Render an R script {#sec-r-test-drive}
...
Get yourself a working R Markdown file, such as the one you made in your [Rmd test drive](sec-rmd-test-drive). Or use the boilerplate `.Rmd` document RStudio makes with *File \> New File \> R Markdown ...*.
...
Drawing on the workflow tips in [Rmd test drive](sec-rmd-test-drive), let's add some YAML frontmatter, properly commented with `#'`, and request `github_document` as the output format. Here's the whole script again:
