# Common remote setups {#sec-common-remote-setups}
...
## Ours (more specifically, yours) {#sec-ours-you}
...
-   Detailed instructions are in [Existing project, GitHub last](sec-existing-github-last).
...
-   Detailed instructions are in [New RStudio Project via git clone](sec-git-clone-usethis-rstudio).
...
What if you do want to make a pull request? This means you should have done *fork-and-clone* instead of *clone*. If you've made no changes or they're easy to save somewhere temporarily, just start over with a fork-and-clone workflow (fully explained in @sec-fork-and-clone) and re-introduce your changes. It is also possible to preserve your work in a local branch, fork the source repo, re-configure your remotes, re-sync up with the source repo, and get back on track. But this is much easier to goof up. And remember to fork and clone (not just clone) in the future!

## Fork (of theirs) {#sec-fork-them}
...
-   Detailed instructions are given in [Fork and clone](sec-fork-and-clone).
...
-   Command line Git or RStudio: You can't complete this task fully from the command line or RStudio:
    -   Fork the source repo in the browser, capture the HTTPS or SSH URL of **your fork**, then use `git clone <FORK_URL>` (command line) or RStudio's *File \> New Project \> Version Control \> Git* workflow. But, wait, you're not done! If you stop here, you will have the incomplete setup we refer to as ["fork (salvageable)"](sec-fork_upstream_is_not_origin_parent), below.
    ...
    -   These last two steps are described in [Finish the fork and clone setup](sec-fork-and-clone-finish).
...
-   In general, it's the same as the regular [fork setup](sec-fork-them) above.
...
You can convert this into the ideal [fork setup](sec-fork-them) like so:

-   Detailed instructions are in [Finish the fork and clone setup](sec-fork-and-clone-finish).
