...
RStudio can only act as a GUI front-end for Git if Git has been successfully installed (@sec-install-git) **AND RStudio can find it**.

A basic test for successful installation of Git is to simply enter `git` in the shell (@sec-shell). If you get a complaint about Git not being found, it means installation was unsuccessful or that it is not being found, i.e. it is not on your `PATH`.
