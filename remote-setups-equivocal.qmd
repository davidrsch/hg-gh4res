# Equivocal remote setups {#sec-equivocal}
...
To identify any of the remote setups described in @sec-common-remote-setups, we need information from GitHub:
...
This means that client packages, like usethis, work best when you have configured a GitHub personal access token (PAT). See @sec-get-a-pat for more details on why and how to do that.
...
Once a PAT is available, this setup can be identified as being ["ours" (belonging to you)](sec-ours-you), ["ours" (but belonging to someone else)](sec-ours-them), or ["theirs"](sec-theirs).
...
Once a PAT is available, this setup can be identified as being a well-configured [fork](sec-fork-them) or a [fork with incomplete setup](sec-fork_upstream_is_not_origin_parent) (or possibly something more weird).

## How to fix

These setups aren't necessarily broken, but usethis needs more information to operate.

To "fix" this, set up a GitHub personal access token. See @sec-get-a-pat for more details on why and how to do that.
